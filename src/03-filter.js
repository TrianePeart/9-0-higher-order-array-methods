...
function getSongsBySaib(songs) {
  return songs.filter(song => song.artist === "Saib" ? song.artist : null )
}
...
function getSongsOverThreeMinutes(songs) {
  return songs.filter(song => 180 < song.runtimeInSeconds ? song.title :null)
}
...
function getTitleTracks(songs) {
  return songs.filter(song => song.title === song.album ? song.title && song.album: null)
}
